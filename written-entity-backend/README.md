...
Elastic is optional for local development. When configured, the `elasticMemory` pipeline step stores each completed meeting in Elasticsearch and searches related prior memories before indexing the current run. Useful demo endpoints:

```bash
GET /api/pipeline/elastic/status
GET /api/pipeline/elastic/search?q=launch%20risks
```

Use `ELASTIC_MCP_SERVER_URL` for the Elastic Agent Builder MCP endpoint copied from Kibana. In Google Cloud Agent Builder, connect that MCP server with an Elasticsearch API key so Gemini can call Elastic-backed search and ES|QL tools during the hackathon demo.
